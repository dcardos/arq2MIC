...
int main(int argc, char *argv[])
{
    ...
    int nThreads = 64;
    if (argc < 2)
        printf("Nenhum argumento encontrado, 64 threads serão disparadas por padrão\n");
    else if (argc == 2){
        nThreads = atoi(argv[1]);
        printf("Serão disparadas %d threads\n", nThreads);
    }
    else{
        printf("Muitos argumentos, usar apenas um com o número de threads.\n");
        exit(-1);
    }
    ...
    args *argumentos = (args *)malloc(nThreads*sizeof(args));
    ...
    int qtdY = iYmax/nThreads;
    int sobra = iYmax % nThreads;
    if (sobra == 0)
        sobra = qtdY;
    else
        sobra = qtdY + sobra;
